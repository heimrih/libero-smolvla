...
class PI0FAST(nn.Module):
    def __init__(self, config: PI0FASTConfig):
        ...
        self.pi0_paligemma.config.ignore_index = config.ignore_index #edited for libero run by heimrih
    ...
    def embed_tokens(self, tokens: torch.Tensor):
        lm = self.pi0_paligemma.language_model
        if hasattr(lm, "model"):
            # older pattern: language_model.model.embed_tokens
            return lm.model.embed_tokens(tokens)
        else:
            # direct pattern: language_model.embed_tokens
